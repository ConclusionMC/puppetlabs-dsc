Function Get-TargetResource {
    ...
    Param(
        ...
        [Parameter(Mandatory=$False)]
        [ValidateNotNullOrEmpty()]
        [string]$Autologon_User,

        [Parameter(Mandatory=$False)]
        [ValidateNotNullOrEmpty()]
        [string]$Autologon_Password,

        [Parameter(Mandatory=$False)]
        [ValidateNotNullOrEmpty()]
        [string]$Autologon_Domain,

        [Parameter(Mandatory=$False)]
        [ValidateNotNullOrEmpty()]
        [string]$Autologon_Count,
    ...
    Elseif ($CheckContent -eq $True) {
        ...
        If ($PSBoundParameters.ContainsKey("Autologon_User") -OR $PSBoundParameters.ContainsKey('Autologon_Password') -OR $PSBoundParameters.ContainsKey('Autologon_Domain') -OR $PSBoundParameters.ContainsKey('Autologon_Count')) {
            $AutoLogon_Start = ($Content | Select-String -Pattern '<AutoLogon>').LineNumber - 1
            $AutoLogon_End = ($Content | Select-String -Pattern '</AutoLogon>').LineNumber - 1
        }
        ...
        If ($PSBoundParameters.ContainsKey('Autologon_User')) {
            $Autologon_UserLine = $UserNameLines | ? { ($_ -gt $AutoLogon_Start) -AND ($_ -lt $AutoLogon_End) }
            $Set_Autologon_User = (($Content[$Autologon_UserLine] -split '<UserName>')[1] -split '</UserName>')[0].Trim()
            If ($Set_Autologon_User -ne $Autologon_User) { $DesiredState = $False }
        }

        If ($PSBoundParameters.ContainsKey('Autologon_Password')) {
            $Autologon_PasswordLine = (($Content | Select-String -Pattern "<Value>") | ? { (($_.LineNumber - 1) -gt $AutoLogon_Start) -AND (($_.LineNumber - 1) -lt $AutoLogon_End) }).LineNumber - 1
            $Set_Autologon_Password = (($Content[$Autologon_PasswordLine] -split '<Value>')[1] -split '</Value>')[0].Trim()
            If ($Set_Autologon_Password -ne $Autologon_Password) { $DesiredState = $False }
        }

        If ($PSBoundParameters.ContainsKey('Autologon_Domain')) {
            $Autologon_DomainLine = $DomainLines | ? { ($_ -gt $AutoLogon_Start) -AND ($_ -lt $AutoLogon_End) }
            $Set_Autologon_Domain = (($Content[$Autologon_DomainLine] -split '<Domain>')[1] -split '</Domain>')[0].Trim()
            If ($Set_Autologon_Domain -ne $Autologon_Domain) { $DesiredState = $False }
        }

        If ($PSBoundParameters.ContainsKey('Autologon_Count')) {
            $Autologon_CountLine = (($Content | Select-String -Pattern "<LogonCount>") | ? { (($_.LineNumber - 1) -gt $AutoLogon_Start) -AND (($_.LineNumber - 1) -lt $AutoLogon_End) }).LineNumber - 1
            $Set_Autologon_Count = [int](($Content[$Autologon_CountLine] -split '<LogonCount>')[1] -split '</LogonCount>')[0].Trim()
            If ($Set_Autologon_Count -ne $Autologon_Count) { $DesiredState = $False }
        }
    ...
    Return @{  
        ...
        Autologon_User         = $Autologon_User
        Autologon_Password     = $Autologon_Password
        Autologon_Domain       = $Autologon_Domain
        Autologon_Count        = $Autologon_Count
...
Function Set-TargetResource {
    ...
    If ( ($CheckContent -eq $True) -OR ($NewFile -eq $True)  ) {
        ...
        If ($PSBoundParameters.ContainsKey('Autologon_User')) {
            $Autologon_UserLine = $UserNameLines | ? { ($_ -gt $AutoLogon_Start) -AND ($_ -lt $AutoLogon_End) }
            $Set_Autologon_User = (($Content[$Autologon_UserLine] -split '<UserName>')[1] -split '</UserName>')[0].Trim()
            If ($Set_Autologon_User -ne $Autologon_User) { $Content[$Autologon_UserLine] = $Content[$Autologon_UserLine].Replace($Set_Autologon_User,$Autologon_User) ; $Update = $True }
        }

        If ($PSBoundParameters.ContainsKey('Autologon_Password')) {
            $Autologon_PasswordLine = (($Content | Select-String -Pattern "<Value>") | ? { (($_.LineNumber - 1) -gt $AutoLogon_Start) -AND (($_.LineNumber - 1) -lt $AutoLogon_End) }).LineNumber - 1
            $Set_Autologon_Password = (($Content[$Autologon_PasswordLine] -split '<Value>')[1] -split '</Value>')[0].Trim()
            If ($Set_Autologon_Password -ne $Autologon_Password) { $Content[$Autologon_PasswordLine] = $Content[$Autologon_PasswordLine].Replace($Set_Autologon_Password,$Autologon_Password) ; $Update = $True }
        }

        If ($PSBoundParameters.ContainsKey('Autologon_Domain')) {
            $Autologon_DomainLine = $DomainLines | ? { ($_ -gt $AutoLogon_Start) -AND ($_ -lt $AutoLogon_End) }
            $Set_Autologon_Domain = (($Content[$Autologon_DomainLine] -split '<Domain>')[1] -split '</Domain>')[0].Trim()
            If ($Set_Autologon_Domain -ne $Autologon_Domain) { $Content[$Autologon_DomainLine] = $Content[$Autologon_DomainLine].Replace($Set_Autologon_Domain,$Autologon_Domain) ; $Update = $True }
        }

        If ($PSBoundParameters.ContainsKey('Autologon_Count')) {
            $Autologon_CountLine = (($Content | Select-String -Pattern "<LogonCount>") | ? { (($_.LineNumber - 1) -gt $AutoLogon_Start) -AND (($_.LineNumber - 1) -lt $AutoLogon_End) }).LineNumber - 1
            $Set_Autologon_Count = [int](($Content[$Autologon_CountLine] -split '<LogonCount>')[1] -split '</LogonCount>')[0].Trim()
            If ($Set_Autologon_Count -ne $Autologon_Count) { $Content[$Autologon_CountLine] = $Content[$Autologon_CountLine].Replace($Set_Autologon_Count,$Autologon_Count) ; $Update = $True }
        }
...
Function Test-TargetResource {
    ...
    If ($CheckContent -eq $True) {
        ...
        If ($PSBoundParameters.ContainsKey('Autologon_User')) {
            $Autologon_UserLine = $UserNameLines | ? { ($_ -gt $AutoLogon_Start) -AND ($_ -lt $AutoLogon_End) }
            $Set_Autologon_User = (($Content[$Autologon_UserLine] -split '<UserName>')[1] -split '</UserName>')[0].Trim()
            If ($Set_Autologon_User -ne $Autologon_User) { Return $False }
        }
        ...
        If ($PSBoundParameters.ContainsKey('Autologon_Password')) {
            $Autologon_PasswordLine = (($Content | Select-String -Pattern "<Value>") | ? { (($_.LineNumber - 1) -gt $AutoLogon_Start) -AND (($_.LineNumber - 1) -lt $AutoLogon_End) }).LineNumber - 1
            $Set_Autologon_Password = (($Content[$Autologon_PasswordLine] -split '<Value>')[1] -split '</Value>')[0].Trim()
            If ($Set_Autologon_Password -ne $Autologon_Password) { Return $False }
        }

        If ($PSBoundParameters.ContainsKey('Autologon_Domain')) {
            $Autologon_DomainLine = $DomainLines | ? { ($_ -gt $AutoLogon_Start) -AND ($_ -lt $AutoLogon_End) }
            $Set_Autologon_Domain = (($Content[$Autologon_DomainLine] -split '<Domain>')[1] -split '</Domain>')[0].Trim()
            If ($Set_Autologon_Domain -ne $Autologon_Domain) { Return $False }
        }

        If ($PSBoundParameters.ContainsKey('Autologon_Count')) {
            $Autologon_CountLine = (($Content | Select-String -Pattern "<LogonCount>") | ? { (($_.LineNumber - 1) -gt $AutoLogon_Start) -AND (($_.LineNumber - 1) -lt $AutoLogon_End) }).LineNumber - 1
            $Set_Autologon_Count = [int](($Content[$Autologon_CountLine] -split '<LogonCount>')[1] -split '</LogonCount>')[0].Trim()
            If ($Set_Autologon_Count -ne $Autologon_Count) { Return $False }
        }
