Function Get-TargetResource {
    ...
    If ((Test-Path "$ODBC\ODBC.INI\$TnsName") -eq $False) { $CreateItems += "$ODBC\ODBC.INI\$TnsName" }
    Else { $Connection = Get-Item "$ODBC\ODBC.INI\$TnsName" }
    ...
    $CreateProperties = @()
    $SetProperties = @()
    If ($DataSources -eq $Null) { $CreateProperties += @{ Path = "$ODBC\ODBC.INI\ODBC Data Sources" ; Name = $TnsName ; Value = "Oracle in $($ORAHome.NAME)" } }
    Else {
        If ($TnsName -notin $DataSources.GetValueNames()) { $CreateProperties += @{ Path = $DataSources.PSPath ; Name = $TnsName ; Value = "Oracle in $($ORAHome.NAME)" } }
        Elseif ($DataSources.GetValue($TnsName) -ne "Oracle in $($ORAHome.NAME)") { $SetProperties += @{ Path = $DataSources.PSPath ; Name = $TnsName ; Value = "Oracle in $($ORAHome.NAME)" } }
    }

    If ($Connection -eq $Null) {
        Foreach ($Property in $Properties.GetEnumerator()) {
            $Key = $Property.Key.Replace('_',' ')
            $CreateProperties += @{ Path = "$ODBC\ODBC.INI\$TnsName" ; Name = $Key ; Value = $Property.Value }
        }
    }
    Else {
