...
Function Set-TargetResource {
    ...
    If ($Changed) { 
        ...
        If ($PSBoundParameters.ContainsKey('WorkDir')) {
            If (Test-Path "$WorkDir\$GpoName.xml") {
                $CurrentStatus = Import-Clixml "$WorkDir\$GpoName.xml"
                $CurrentStatus.ModificationTime = (Get-GPO -Name $GpoName).ModificationTime
                $CurrentStatus | Export-Clixml -Path "$WorkDir\$GpoName.xml" -Force
            }
        }
    }
}
